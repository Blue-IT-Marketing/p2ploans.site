...
document.getElementById('SendMessageButt').addEventListener("click", function (ev) {
            var vstrSelector = "Message";
            var dataString = '&vstrSelector=' + vstrSelector;
            ...
            $.ajax({
                ...
                cache: false,
                success: function (html) {
                    $('#FinanciersINFDIV').html(html)
                }
            });
});

document.getElementById('SendLoanRequestButt').addEventListener("click", function (ev) {
            var vstrSelector = "LoanRequest";
            var dataString = '&vstrSelector=' + vstrSelector;
            var sendToURL = "/p2ploans/financiers/messages";
